...
# Initialize NLTK only when needed
...
def get_db_connection():
    ...
    if not hasattr(_local, 'connection'):
        _local.connection = sqlite3.connect('database.db', check_same_thread=False)
        # Set up WAL mode for better concurrency
        _local.connection.execute('PRAGMA journal_mode=WAL')
        _local.connection.execute('PRAGMA synchronous=NORMAL')
        _local.connection.execute('PRAGMA cache_size=10000')  # Increase cache
        _local.connection.execute('PRAGMA temp_store=MEMORY')  # Use memory for temp tables
...
def inject_user_logo():
    """Inject user logo into all templates"""
    if current_user.is_authenticated:
        with get_db_connection() as conn:
            c = conn.cursor()
            
            if current_user.role == 'creator':
                # For instructors, show their own logo
                ...
                if result and result[0]:
                    ...
                    if not logo_path.startswith('/'):
                        logo_path = '/' + logo_path
                    if os.path.exists(logo_path.lstrip('/')):
                        return {'user_logo': logo_path}
            
            elif current_user.role == 'student':
                # For students, show the logo of their enrolled course's instructor
                # Get the first enrolled course's instructor logo
                c.execute('''
                    SELECT u.logo 
                    FROM enrollments e 
                    JOIN courses c ON e.course_id = c.id 
                    JOIN users u ON c.creator_id = u.id 
                    WHERE e.user_id = ? AND e.approval_status = 1 
                    LIMIT 1
                ''', (current_user.id,))
                result = c.fetchone()
                if result and result[0]:
...
def generate_enrollment_number():
    return f"EN{random.randint(100000, 999999)}"
...
def create_notification(user_id, message, link=None, notif_type=None, course_id=None):
    conn = sqlite3.connect('database.db')
    c = conn.cursor()
    ...
    if notif_type:
        ...
        if prefs:
            ...
            if notif_type == 'announcements' and not prefs[2]:
                conn.close()
                return
    c.execute('INSERT INTO notifications (user_id, message, link, course_id) VALUES (?, ?, ?, ?)', (user_id, message, link, course_id))
    conn.commit()
    conn.close()
...
def notifications():
    ...
    if current_user.role == 'student':
        # For students, only show notifications from courses they are enrolled in
        c.execute('''
            SELECT n.id, n.message, n.link, n.is_read, n.created_at, n.course_id, c.title
            FROM notifications n
            LEFT JOIN courses c ON n.course_id = c.id
            WHERE n.user_id = ? AND (n.course_id IS NULL OR n.course_id IN (
                SELECT course_id FROM enrollments WHERE user_id = ? AND approval_status = 1
            ))
        ...
        c.execute('''
            ...
            WHERE n.user_id = ? AND n.is_read = 0 AND (n.course_id IS NULL OR n.course_id IN (
                SELECT course_id FROM enrollments WHERE user_id = ? AND approval_status = 1
            ))
        ''', (current_user.id, current_user.id))
    else:
        # For instructors and admins, show all notifications
        c.execute('SELECT id, message, link, is_read, created_at, course_id FROM notifications WHERE user_id = ? ORDER BY created_at DESC', (current_user.id,))
        notifications = c.fetchall()
...
def home_dashboard():
    ...
    if current_user.role == 'creator':
        ...
        c.execute('''
            SELECT c.id, c.title, c.description, c.creator_id, c.meeting_link, c.course_code 
            FROM courses c 
            JOIN users u ON c.creator_id = u.id 
            WHERE c.creator_id = ? AND (u.is_blocked = 0 OR u.is_blocked IS NULL)
        ''', (current_user.id,))
        courses = c.fetchall()
        enrolled_courses = []
        creator_courses = [row[0] for row in courses]
    elif current_user.role == 'student':
        # Students see all courses they are enrolled in (approved and pending)
        c.execute('''
            SELECT c.id, c.title, c.description, c.creator_id, c.meeting_link, c.course_code, e.approval_status
            FROM courses c 
            JOIN enrollments e ON c.id = e.course_id 
            JOIN users u ON c.creator_id = u.id 
            WHERE e.user_id = ? AND (u.is_blocked = 0 OR u.is_blocked IS NULL)
        ''', (current_user.id,))
        courses = c.fetchall()
        enrolled_courses = [row[0] for row in courses]
        creator_courses = []
    else:
    ...
    if current_user.is_authenticated:
        # For students, only check assignments for approved enrolled courses
        if current_user.role == 'student':
            # Only get assignments from courses where student is approved
            c.execute('SELECT course_id FROM enrollments WHERE user_id = ? AND approval_status = 1', (current_user.id,))
            enrolled_courses_for_assignments = [row[0] for row in c.fetchall()]
        else:
            # For creators and admins, check all their courses
            ...
            if current_user.role == 'creator':
                ...
                enrolled_courses_for_assignments.extend([row[0] for row in c.fetchall()])
        import datetime
        today = datetime.date.today().isoformat()
        for course_id in enrolled_courses_for_assignments:
        ...
        # Fetch unread notifications count (filtered for students)
        if current_user.role == 'student':
            c.execute('''
                SELECT COUNT(*) FROM notifications n
                WHERE n.user_id = ? AND n.is_read = 0 AND (n.course_id IS NULL OR n.course_id IN (
...
def register():
    if request.method == 'POST':
        ...
        if not payment_screenshot or not payment_screenshot.filename:
            flash('Payment screenshot is required.')
            return render_template('register.html')
        
        # Validate course code
        conn = sqlite3.connect('database.db')
        c = conn.cursor()
        c.execute('SELECT id, creator_id FROM courses WHERE course_code = ?', (course_code,))
        course_data = c.fetchone()
        if not course_data:
            conn.close()
            flash('Invalid course code. Please check the code provided by your instructor.')
            return render_template('register.html')
        ...
        import time
        filename = f"payment_{username}_{int(time.time())}.{payment_screenshot.filename.split('.')[-1]}"
        payment_screenshot.save(os.path.join(app.config['UPLOAD_FOLDER'], filename))
        
        # Create user account (students are auto-approved)
        hashed_password = generate_password_hash(password)
        c.execute('INSERT INTO users (username, password, role, full_name, email, phone, institution, is_approved, payment_screenshot) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
                  (username, hashed_password, 'student', full_name, email, phone, institution, 1, filename))
        user_id = c.lastrowid
        
        # Create enrollment with pending instructor approval
        enrollment_number = generate_enrollment_number()
        c.execute('INSERT INTO enrollments (user_id, course_id, enrollment_number, approval_status) VALUES (?, ?, ?, ?)',
                  (user_id, course_id, enrollment_number, 0))
        
        conn.commit()
        conn.close()
        flash('Registration successful! Please wait for instructor approval to access course content.')
        return redirect(url_for('login'))
    return render_template('register.html')
...
def login():
    if request.method == 'POST':
        ...
        if user_data and check_password_hash(user_data[2], password):
            ...
            # Check if user is approved (only for instructors, not students)
            if user_data[3] == 'creator' and user_data[4] != 1:
                flash('Your instructor account is pending admin approval.')
...
def create_course():
    ...
    if request.method == 'POST':
        ...
        c.execute('INSERT INTO courses (title, creator_id, description, reference_file_path, meeting_link, outline, objectives, live_session_start, live_session_end, live_days, course_code) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
                  (title, current_user.id, description, None, meeting_link, outline, objectives, live_session_start, live_session_end, live_days_str, course_code))
...
def edit_course(course_id):
    ...
    conn = sqlite3.connect('database.db')
    c = conn.cursor()
    c.execute('SELECT id, title, description, creator_id, reference_file_path, meeting_link, live_session_start, live_session_end, live_days, outline, objectives FROM courses WHERE id = ?', (course_id,))
    course = c.fetchone()
    if not course:
        conn.close()
    ...
    if request.method == 'POST':
        ...
        c.execute('UPDATE courses SET title = ?, description = ?, meeting_link = ?, outline = ?, objectives = ?, live_session_start = ?, live_session_end = ?, live_days = ? WHERE id = ?',
                  (title, description, meeting_link, outline, objectives, live_session_start, live_session_end, live_days_str, course_id))
        ...
        # Handle chapter files (same logic as create_course)
        chapter_files = {}
        for key in request.files:
            if key.startswith('chapter_files_'):
                chapter_num = int(key.split('_')[-1]) - 1  # Convert to 0-based index
    ...
    # Parse live_days string back to list
    live_days = course[8].split(',') if course[8] else []
    ...
    return render_template('edit_course.html', 
                         ...
                         meeting_link=course[5],
                         live_session_start=course[6],
                         live_session_end=course[7],
                         live_days=live_days,
                         outline=course[9],
                         objectives=course[10])
...
def forum(course_id):
    ...
    c.execute('SELECT enrollment_number, approval_status FROM enrollments WHERE user_id = ? AND course_id = ?', (current_user.id, course_id))
    enrollment = c.fetchone()
    if not enrollment and current_user.role != 'creator':
        conn.close()
        flash('You must be enrolled or be the creator to view forum.')
        return redirect(url_for('home'))
    
    # Check if student is approved for this course
    if current_user.role == 'student' and enrollment and enrollment[1] != 1:
        conn.close()
        flash('You must be approved by the instructor to access this course content.')
        return redirect(url_for('home'))
...
def serve_upload(filename):
    """Serve uploaded files including logos"""
    try:
...
def enroll(course_id):
    ...
    if not course_data:
        ...
        return redirect(url_for('home'))
    course_title = course_data[0]
    c.execute('SELECT id, approval_status FROM enrollments WHERE user_id = ? AND course_id = ?', (current_user.id, course_id))
    enrollment_data = c.fetchone()
    if enrollment_data:
        if enrollment_data[1] == 1:
            conn.close()
            flash('You are already enrolled in this course.')
            return redirect(url_for('home'))
        elif enrollment_data[1] == 0:
            conn.close()
            flash('Your enrollment is pending instructor approval.')
            return redirect(url_for('home'))
        elif enrollment_data[1] == -1:
            conn.close()
            flash('Your enrollment was rejected by the instructor.')
            return redirect(url_for('home'))
    
    if request.method == 'POST':
        enrollment_number = generate_enrollment_number()
        c.execute('INSERT INTO enrollments (user_id, course_id, enrollment_number, approval_status) VALUES (?, ?, ?, ?)',
                  (current_user.id, course_id, enrollment_number, 0))
        conn.commit()
        conn.close()
        flash('Enrollment request submitted! Please wait for instructor approval.')
        return redirect(url_for('home'))
    conn.close()
...
def progress(course_id):
    ...
    try:
        ...
        c.execute('SELECT enrollment_number, approval_status FROM enrollments WHERE user_id = ? AND course_id = ?',
                  (current_user.id, course_id))
        enrollment = c.fetchone()
        if not enrollment and current_user.role != 'creator':
...
def edit_progress(course_id, progress_id):
    ...
    for attempt in range(max_retries):
        ...
        try:
            ...
            try:
                create_notification(student_id, f'Your assignment grade was updated in {course_title}', url_for('progress', course_id=course_id), 'grades', course_id)
            except:
    ...
    if request.method == 'POST' and is_self:
        ...
        if logo_file and current_user.role == 'creator':
            if logo_file.filename and logo_file.filename.lower().endswith(('.png', '.jpg', '.jpeg')):
                ...
                if file_size <= 2 * 1024 * 1024:  # 2MB limit
                    ...
                    c.execute('UPDATE users SET logo = ? WHERE id = ?', (filepath_normalized, current_user.id))
                    conn.commit()
                    flash('Logo updated!')
                else:
        ...
        if 'generate_token' in request.form:
            ...
            if current_user.role != 'admin':
                flash('Only administrators can generate API tokens.')
                conn.close()
        ...
        if 'revoke_token' in request.form:
            # Only allow admin users to revoke API tokens
            if current_user.role != 'admin':
                flash('Only administrators can revoke API tokens.')
                conn.close()
                return redirect(url_for('profile'))
...
def dashboard():
    ...
    if current_user.role == 'creator':
        ...
        # Upcoming assignments (only from approved enrollments)
        c.execute('''SELECT a.name, a.due_date, c.title FROM assignments a 
                     JOIN enrollments e ON a.course_id=e.course_id 
                     JOIN courses c ON a.course_id=c.id 
                     WHERE e.user_id=? AND e.approval_status=1 AND a.due_date>=date('now') 
                     ORDER BY a.due_date''', (current_user.id,))
        upcoming_assignments = [{'name': row[0], 'due_date': row[1], 'course': row[2]} for row in c.fetchall()]
        conn.close()
...
def pending_approvals():
    ...
    conn = sqlite3.connect('database.db')
    c = conn.cursor()
    # Admin only deals with instructor approvals
...
def admin_dashboard():
    ...
    # Get platform statistics (admin focuses on instructors only)
    ...
    c.execute('SELECT COUNT(*) FROM users WHERE role = "creator" AND is_approved = 1')
    approved_instructors = c.fetchone()[0]
    ...
    approved_instructors_list = c.fetchall()
    ...
    return render_template('admin_dashboard.html', 
                         ...
                         approved_instructors_list=approved_instructors_list)
...
def instructor_enrollments():
    ...
    for course in courses:
        c.execute('''SELECT enrollments.id, users.full_name, users.username, enrollments.approval_status, courses.title, enrollments.user_id, enrollments.course_id, users.avatar, users.is_blocked, users.payment_screenshot
                     FROM enrollments
        ...
        for enrollment in course_enrollments:
            ...
            if user_id not in users_data:
                users_data[user_id] = {
                    ...
                    'payment_screenshot': enrollment[9]
                }
...
def approve_enrollment(enrollment_id):
    ...
    conn = sqlite3.connect('database.db')
    c = conn.cursor()
    c.execute('UPDATE enrollments SET approval_status=1 WHERE id=?', (enrollment_id,))
    conn.commit()
    conn.close()
